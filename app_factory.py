...
def init_user(state):
    try:
        ...
        user_agent = init_user_chatbot_agent(uuid_str=uuid_str)
        #给agent中的qr_tool添加state属性
        qr_tool = user_agent.available_tool_list['QRcode_beautify']
        if qr_tool is not None:
            setattr(qr_tool, 'state', state)
...
with demo:
    ...
    def upload_file(chatbot, file, _state):
